...
describe "BR bank holidays" do
  ...
  %w(
    ...
    it "treats #{date} as not a business day" do
      expect(BusinessCalendar.for(:BR).is_business_day?(date)).to be false
    end
  end

  it "calculates previous business day correctly" do
    expect(BusinessCalendar.for(:BR).preceding_business_day(Date.parse('2018-11-19'))).to eq (Date.parse('2018-11-16'))
  end

  it "calculates next business day correctly" do
    expect(BusinessCalendar.for(:BR).following_business_day(Date.parse('2018-11-19'))).to eq (Date.parse('2018-11-21'))
  end
end
